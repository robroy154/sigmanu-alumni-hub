...
export default async function AdminRegistrationsPage({ searchParams }: Props) {
  ...
  const totalAttendees = rows.reduce((sum, r) => sum + 1 + (r.guest_count ?? 0), 0);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap gap-4 text-sm">
        ...
        <span className="text-sn-gray-text">
          <span className="text-sn-off-white font-medium">{totalAttendees}</span> attendee{totalAttendees !== 1 ? "s" : ""}
        </span>
        <span className="text-sn-gray-text">
          Revenue:{" "}
