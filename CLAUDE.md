...
- Phase 12: Auth hardening — forgot password flow (PKCE via /auth/callback?next=/auth/reset-password), reset password page, duplicate email interception on signup
...
- Password reset flow uses PKCE: ForgotPasswordForm sends redirectTo=${APP_URL}/auth/callback?next=/auth/reset-password; /auth/callback exchanges the code via exchangeCodeForSession then short-circuits directly to next when next.startsWith("/auth/") (skips member status checks); ResetPasswordForm renders immediately in ready state and calls updateUser({ password }) — no onAuthStateChange/PASSWORD_RECOVERY listener; open-redirect guard in callback ensures next must start with "/"
