...
- Stub claim in signup: both SignupForm and JoinForm call findStubMatches() before signUp(); stub claim UI extracted into shared StubClaimStep component (src/components/auth/StubClaimStep.tsx); if matches found, pauses signup and shows up to 3 cards; user selects "This is me" or "None of these are me"; stub_id passed via signUp options.data; pin_number never masked anywhere; JoinForm passes firstName+lastName+pledgeClass (no badge number field on join form by design); SignupForm additionally passes pinEntry (badge number search hint)
...
- Admin member detail big brother display: resolvedBigName resolved server-side in page.tsx across all statuses (including stubs); if big_id points to a stub not in the dropdown, shows "Currently linked to: [Name] (unclaimed stub)" note below the select; dropdown options remain member+admin only (stubs excluded from selection)
...
- Family tree fitView fix (Phase 22 patch): fitView prop fires before React Flow mounts 258+ stub nodes → blank canvas. Fixed by removing fitView prop; useEffect + hasFitViewRef defers rf.fitView() 100ms after first node count change so all node positions are registered before fit
- Family tree hide-isolated default: hideIsolated state initializes to true (hides unlinked on load); toggle button label describes the action to be taken ("Show Unlinked" when filter active, "Hide Unlinked" when not)
- Family tree stub node layout: NODE_W=190, NODE_H=88; "Unclaimed" badge at top-1.5 right-1.5 with pointer-events-none z-10; info div gets pr-16 when is_stub to prevent name text reaching badge area; pin displayed as ΜΞ 001 below pledge class row
