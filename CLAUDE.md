...
Phase 22 complete — pre-seeded alumni stub system: stub status, pg_trgm fuzzy search, signup claim flow, family tree stub nodes, admin CSV import. handle_new_user trigger hotfixed — stub claim errors no longer block signup (EXCEPTION block wraps stub path; normal insert always runs as fallback).
