...
- Email: RESEND_API_KEY required (from resend.com); RESEND_FROM_EMAIL required (verified sender on a Resend domain); all emails send with display name "Mu Xi Chapter of Sigma Nu Fraternity" via SENDER_NAME/SENDER_EMAIL constants; Resend SDK (resend npm package); migrated from Brevo (@getbrevo/brevo) back to Resend in Phase 25 — Resend does not rewrite links, which is required for Supabase auth emails to work correctly via SMTP relay
