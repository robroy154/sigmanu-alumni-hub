...
- handle_new_user trigger updated: reads stub_id from raw_user_meta_data; if found, UPDATE stub row in place (sets id=new.id, email, first_name, last_name, status='pending', updated_at) — no INSERT+DELETE, eliminates unique constraint violations on pin_number; normal path unchanged
