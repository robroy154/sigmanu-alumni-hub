...
- RichTextContent.tsx HTML sanitization: uses `isomorphic-dompurify` (not regex); DOMPurify v3 with JSDOM polyfill for Node.js server components; module-level `afterSanitizeAttributes` hook removes non-YouTube iframes and forces `target=_blank rel=noopener noreferrer` on all links; `FORCE_BODY: true` ensures valid document structure
